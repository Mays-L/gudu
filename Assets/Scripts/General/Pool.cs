...
public class Pool : MonoBehaviour
{
    ...
    void InitializePoolsOnStart()
    {
        ...
        for (int i = 0; i < pools.Length; i++)
        {
            ...
            _parent = new GameObject(nameOfPool);
        ...
        (string nameOfObject, Vector3 pos, Quaternion quaternion)
    {
        ...
        if (_poolsContent.ContainsKey(nameOfObject))
        {
            ...
            if (poolCurrentSize > 0)
            {
                ...
                returnGameObject.transform.rotation = quaternion;
            ...
            else
            {
                for(int i = 0; i < pools.Length; i++)
                {
                    if (pools[i].nameOfPool == nameOfObject)
                    {
                        ...
                        returnGameObject.name = pools[i].nameOfPool;
                        returnGameObject.transform.SetParent(_parents[nameOfObject].transform);
                        return returnGameObject;
                    }
                }
        ...
        {
            Debug.LogError("The name isn't in the pool.");
            return null;
        }
    }

    public static void DestroyGameObject(string gameObjectName, GameObject destroyedGameObject)
    {
        if (_poolsContent.ContainsKey(gameObjectName))
        {
            destroyedGameObject.SetActive(false);
            _poolsContent[gameObjectName].Add(destroyedGameObject);
        }
        else
        {
            Debug.LogError("The Game Object name doesn't exist in the pool list... " + gameObjectName);
        }
    }
